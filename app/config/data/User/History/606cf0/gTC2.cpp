...
std::vector<Point3D> backprojectToCameraSpace(const cv::Mat& depthMap, float fx, float fy, float cx, float cy) {
    std::vector<Point3D> points;

    for (int v = 0; v < depthMap.rows; ++v) {
        for (int u = 0; u < depthMap.cols; ++u) {
            float d = depthMap.at<float>(v, u);  // depth in meters
...
int main()
{
	...
	while (sensor.ProcessNextFrame())
	{
		...
		// Load depth image (assume it's a 32-bit float image in meters)
		cv::Mat depth = cv::imread("depth.png", cv::IMREAD_UNCHANGED);
		depth.convertTo(depth, CV_32F, 1.0 / 1000.0);  // convert from mm to meters if needed

		// Camera intrinsics
		float fx = 525.0f, fy = 525.0f;
		float cx = 319.5f, cy = 239.5f;

		std::vector<Point3D> cameraPoints = backprojectToCameraSpace(depth, fx, fy, cx, cy);

		std::cout << "Back-projected " << cameraPoints.size() << " 3D points.\n";
