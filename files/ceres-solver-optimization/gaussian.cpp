...
struct GaussianCostFunction
{
	...
	// The point for which the cost function is computed
	template<typename T>
	bool operator()(const T* const mu, const T* const sigma, T* residual) const {

		// Explanation:
		// mu[0] and sigma[0] are the parameters ceres will optimize

		T norm = T(1.0) / (ceres::sqrt(T(2.0) * T(M_PI)) * sigma[0]);
		T exponent = -ceres::pow(point.x - mu[0], 2) / (T(2.0) * ceres::pow(sigma[0], 2));
		T model = norm * ceres::exp(exponent);

		// minimize the residual
		residual[0] = T(point.y) - model;
		return true;
	}
