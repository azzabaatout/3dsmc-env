...
#pragma omp parallel for
        for (int v = 1; v < height - 1; ++v) {
            for (int u = 1; u < width - 1; ++u) {
                ...
                // Old code: normalsTmp[idx] = Vector3f(1, 1, 1); // Needs to be replaced.
                // we already have du and dv which are the depth derivatives

                // we create 2 tan vectors to the surface at the point (u, v) in 3d space
                Vector3f tu(1.0f, 0.0f, du); // Tangent vector in x direction
                Vector3f tv(0.0f, 1.0f, dv); // Tangent vector in y direction
                normalsTmp[idx] = tu.cross(tv); // The normal vector n is perpendicular to both tangents so it s given by the cross product
